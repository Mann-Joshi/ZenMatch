...
export function getBlockers(tile: Tile, buckets: TileBuckets): { blockedAbove: boolean; blockedLeft: boolean; blockedRight: boolean } {
  ...
  for (let y = centerY - ySteps; y <= centerY + ySteps; y += 1) {
    for (let x = centerX - xSteps; x <= centerX + xSteps; x += 1) {
      ...
      for (let i = 0; i < bucket.length; i++) {
        ...
        if (!blockedAbove && candidate.z > tile.z) {
          if (candidate.y === tile.y && candidate.x === tile.x) {
            blockedAbove = true;
          }
        }
...
export function computeFreeTiles(tiles: Tile[]): Tile[] {
  ...
  return tiles.map((tile) => {
    ...
    const { blockedAbove, blockedLeft, blockedRight } = getBlockers(tile, buckets);
...
export function getHintPair(tiles: Tile[]): [string, string] | null {
  ...
  for (let p = 0; p < pairsFound.length; p++) {
    ...
    for (const tile of candidatesToCheck) {
      ...
      const { blockedAbove, blockedLeft, blockedRight } = getBlockers(tile, initialBuckets);
      const isFree = !blockedAbove && (!blockedLeft || !blockedRight);
