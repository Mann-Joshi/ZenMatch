...
function isTileFree(tile: Tile, buckets: TileBuckets): boolean {
  const yCenter = Math.round(tile.y * POSITION_SCALE);
  const xCenter = Math.round(tile.x * POSITION_SCALE);
  ...
  for (let y = yCenter - ySteps; y <= yCenter + ySteps; y += 1) {
    for (let x = xCenter - xSteps; x <= xCenter + xSteps; x += 1) {
...
export function getHintPair(tiles: Tile[]): [string, string] | null {
  ...
  for (let p = 0; p < pairsFound.length; p++) {
    ...
    const checkNearby = (tile: Tile) => {
      ...
      for (let y = yCenter - ySteps; y <= yCenter + ySteps; y += 1) {
        for (let x = xCenter - xSteps; x <= xCenter + xSteps; x += 1) {
          const b = initialBuckets.get(`${y}:${x}`);
          if (!b) continue;
          for (let i = 0; i < b.length; i++) {
            if (!b[i].isMatched) candidatesToCheck.add(b[i]);
          }
        }
      }
    };
    ...
    for (const tile of candidatesToCheck) {
      ...
      if (isTileFree(tile, initialBuckets)) {
        nextFreeTilesCount++;
      }
    }
